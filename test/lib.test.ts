...
test('tags', async () => {
  ...
  expect(await resolver.resolveInverseTags('components/boxshot.js')).toEqual(new Set(['boxshot', 'browse', 'search']));
...
test('tests', async () => {
  ...
  const tests = await resolver.resolveTests('components/keyboard.js');
